 ...
 */
function removeBlock(lines, startPattern, endPattern) {
    const result = [];
    let inBlock = false;
 ...
 */
function removeInterface(lines, interfaceName) {
...
const POLYMATH_IMPORT_LINE = "import type { IExtension, IToolkit, AppEntry, ToolDefinition, ToolResponse, ToolProgress } from '@polymath/types'";
...
function fixPolymathImport(lines, appliedFixes) {
...
`;
function fixParametersToInputSchema(source, appliedFixes) {
...
function fixCallToolArgsType(source, appliedFixes) {
 ...
 */
function injectArgsCast(source, originalTypeName) {
 ...
 */
function detectOriginalArgsType(source) {
...
function fixAddStreamingOverload(source, appliedFixes) {
    ...
    result = result.replace(/(async\s+callTool\s*\(\s*)(name\s*:\s*string\s*,\s*)(\w+\s*:\s*Record<string,\s*unknown>)(\s*\)\s*(?::\s*Promise<ToolResponse>)?\s*\{)/, (match, asyncKw, nameParam, argsParam, rest) => {
...
`;
    result = result.replace(/(\s*)(private\s+async\s+_callImpl\s*\()/, (match, indent, rest) => `${overloadBlock}${indent}${rest}`);
    if (result !== source) {
        appliedFixes.push("Added streaming callTool overload (delegates to _callImpl)");
    }
    return result;
}
export function fixFile(filePath, outPath, issues, opts = {}) {
