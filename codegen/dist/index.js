...
function writeCMakeLists(descriptor, srcDir, outFile, polybindRoot, noBridge = false) {
  const soName = descriptor.extension.so_name
  ...
  // Recursively collect all .cpp files relative to srcDir
  function walkCpp(dir, prefix) {
    const results = []
    try {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        const rel = prefix ? prefix + '/' + entry.name : entry.name
        if (entry.isDirectory()) {
          results.push(...walkCpp(join(dir, entry.name), rel))
  ...
  let sources = []
  try {
    const cppFiles = walkCpp(srcDir, '')
    sources = (cppFiles.length > 0 ? cppFiles : ['polybind_entry.cpp'])
      .map(f => {
        // Try to resolve the source file to its real path (follow symlinks).
        // If it lives on a Windows drive (/mnt/X/...), emit X:/... so that
...
function cmdCmake(args) {
  ...
  if (!yamlPath || !outFile) { console.error('Usage: polybind cmake <tools.yaml> --polybind-root <dir> --out <CMakeLists.txt> [--no-bridge]'); process.exit(1) }
  const absYaml = resolve(process.cwd(), yamlPath)
  const absPolybindRoot = resolve(process.cwd(), polybindRoot)
  const absOut = resolve(process.cwd(), outFile)
  mkdirSync(dirname(absOut), { recursive: true })
  const descriptor = parseToolsYaml(absYaml)
  writeCMakeLists(descriptor, dirname(absYaml), absOut, absPolybindRoot, noBridge)
...
function cmdWrappers(args) {
  ...
  const descriptor = parseToolsYaml(absYaml)
  const platforms = descriptor.extension.platforms || ['node', 'arkts']
  if (platforms.includes('node')) { writeNodeWrapper(descriptor, absOut); console.log(`✓ PolybindExtension.js`) }
