 ...
 */
function checkPolymathImport(lines, issues) {
 ...
 */
function checkLocalInterfaceDefinitions(lines, issues) {
 ...
 */
function checkIExtensionClass(lines, issues) {
 ...
 */
function checkIToolkitClass(lines, issues) {
 ...
 */
function checkCallToolSignatures(lines, issues) {
    ...
    const hasTypedArgs = callToolLines.some(l => /callTool\s*\(\s*\w+\s*:\s*string\s*,\s*\w+\s*:\s*(?!Record)/.test(l));
    if (hasTypedArgs) {
        const line = findLine(lines, /callTool\s*\(\s*\w+\s*:\s*string\s*,\s*\w+\s*:\s*(?!Record)/);
        issues.push({
            severity: 'error',
    ...
    const hasStreamingOverload = hasPattern(lines, /callTool\s*\([\s\S]*?progress\s*:\s*true/) || hasPattern(lines, /callTool\s*\([\s\S]*?,\s*true\s*\)/);
 ...
 */
function checkInputSchema(lines, issues) {
